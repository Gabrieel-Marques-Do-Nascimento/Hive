...
"""
    ...
    >>> data_base: dict = {'destinatario_id': int, 'mensagem': str, 'id': int}
        """
try:
socket_logger.info(f"Saving message: {
	data_base
}")
...
destinatario_message = Messages(
	user = destinatario, message = data_base["message"], other_Id = data_base["id"], to = data_base["to"])
...
socket_logger.critical(f"Error: {
	e
}")
emit("error", {
