...
def socket_register(socketio: SocketIO):
	...
	def channel(data: dict):
		...
		msg_db= Messages(user=user, message=data["message"], pessoaId=data.get("d-id"), senderId=data.get("id"))
		
		dest_msg_db = Messages(user=user,pessoaId=data.get("d-id"), message=data.get("message"), senderId=data.get("id"))
